...
// HasPotentialAccountForRequest reports whether the current group/platform still
// has at least one account that can theoretically serve the requested model once
// temporary runtime blocks or rate limits clear.
func (s *GatewayService) HasPotentialAccountForRequest(ctx context.Context, groupID *int64, requestedModel string) (bool, error) {
	group, resolvedGroupID, err := s.resolveGatewayGroup(ctx, groupID)
	if err != nil {
		return false, err
	}
	if s.checkChannelPricingRestriction(ctx, resolvedGroupID, requestedModel) {
		return false, nil
	}
	...
	accounts, useMixed, err := s.listPotentialAccounts(ctx, resolvedGroupID, platform, hasForcePlatform)
	if err != nil {
		return false, err
	}
	for i := range accounts {
...
func (s *GatewayService) listPotentialAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	...
	useMixed := !hasForcePlatform && (platform == PlatformAnthropic || platform == PlatformGemini)
	if groupID != nil {
	...
	for _, account := range accounts {
		if s.isAccountInGroup(&account, nil) {
