	...
	for {
		...
		if err != nil {
			if len(fs.FailedAccountIDs) == 0 {
				...
				if retryErr != nil {
					reqLog.Warn("gateway.cc.account_retry_check_failed", zap.Error(retryErr))
				} else if retrySelection {
					reqLog.Info("gateway.cc.account_select_waiting_for_available_account",
					...
					if waitErr != nil {
						reqLog.Info("gateway.cc.account_select_wait_interrupted", zap.Error(waitErr))
						return
					}
					if waited {
